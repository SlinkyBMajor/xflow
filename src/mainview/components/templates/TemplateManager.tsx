...
export function TemplateManager({ onApply, onClose }: TemplateManagerProps) {
	...
	return (
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
			<div className="bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl w-[560px] max-h-[80vh] flex flex-col">
				{/* Header */}
				<div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800/60">
					<h2 className="text-sm font-semibold text-zinc-200">Board Templates</h2>
					...
					{showExport ? (
						<div className="space-y-2">
							...
							<div className="flex gap-2 pt-1">
								<Button size="sm" onClick={handleExport} className="bg-violet-600 hover:bg-violet-500 text-white">
									Export
								</Button>
						...
						<Button
							...
							className="text-zinc-400 hover:text-zinc-200 w-full"
						>
...
function TemplateCard({
	...
	return (
		<div className="p-3.5 rounded-lg bg-zinc-800/40 border border-zinc-800 hover:border-zinc-700 transition-colors">
			<div className="flex items-start justify-between mb-2">
				<div>
					<h3 className="text-[13px] font-medium text-zinc-200">{template.name}</h3>
					{template.builtIn && (
						<span className="text-[9px] font-mono text-zinc-500 uppercase tracking-wider">Built-in</span>
					)}
				</div>
			</div>

			{template.description && (
				<p className="text-[11px] text-zinc-500 mb-2 line-clamp-2">{template.description}</p>
			)}
			...
			<div className="flex gap-1.5">
				<Button
					...
					onClick={onApply}
					disabled={applying}
					className="flex-1 h-7 text-xs bg-violet-600/80 hover:bg-violet-500 text-white"
				>
					Apply
				</Button>
				{onDelete && (
					<Button
						...
						variant="ghost"
						onClick={onDelete}
						className="h-7 text-xs text-zinc-500 hover:text-red-400 hover:bg-red-950/30 px-2"
					>
